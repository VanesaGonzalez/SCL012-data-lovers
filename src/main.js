...
function makeChampions(allChamp) {
  ...
  allChamp.map(champion => {
    innerHTML += '<div class = "championBox margin-top">';
    innerHTML += '<button class="imageButton center"><img class="imgRedonda" src = "' + champion.splash + '"/></button>';
    innerHTML += '<p class="name-title">' + champion.name + '</p>';
    innerHTML += '<p class="name-title">' + champion.title + '</p>';
